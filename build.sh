...
source ${TOP}/device/nexell/cluster/common.sh
...
revert_common ${TOP}/device/nexell/cluster/patch
patch_common ${TOP}/device/nexell/cluster/patch
...
DTB_IMG=${KERNEL_DIR}/arch/arm/boot/dts/s5p4418-cluster_ref-rev00.dtb
...
if [ "${BUILD_ALL}" == "true" ] || [ "${BUILD_BL1}" == "true" ]; then
	build_bl1_s5p4418 ${BL1_DIR}/bl1-${TARGET_SOC} nxp4330 cluster 3
fi

if [ "${BUILD_ALL}" == "true" ] || [ "${BUILD_UBOOT}" == "true" ]; then
	build_uboot ${UBOOT_DIR} ${TARGET_SOC} ${BOARD_NAME}_ref ${CROSS_COMPILE}
...
if [ "${BUILD_ALL}" == "true" ] || [ "${BUILD_KERNEL}" == "true" ]; then
	build_kernel ${KERNEL_DIR} ${TARGET_SOC} ${BOARD_NAME} s5p4418_cluster_ref_nougat_defconfig ${CROSS_COMPILE}
	test -d ${OUT_DIR} && \
...
if [ -f ${UBOOT_DIR}/u-boot.bin ]; then
	...
	UBOOT_BOOTARGS="console=ttyAMA3,115200n8 printk.time=1 androidboot.hardware=cluster androidboot.console=ttyAMA3 androidboot.serialno=0123456789ABCFEF"
...
make_ext4_recovery_image \
	...
	${KERNEL_DIR}/arch/arm/boot/dts/s5p4418-cluster_ref-rev00.dtb \
